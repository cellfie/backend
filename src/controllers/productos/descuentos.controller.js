...
export const createDescuento = async (req, res) => {
  ...
  try {
    ...
    await pool.query("UPDATE descuentos SET activo = 0 WHERE producto_id = ? AND activo = 1", [producto_id])

    // Usar la función utilitaria para obtener la fecha actual en Argentina
    const fechaActual = formatearFechaParaDB()

    // Insertar el nuevo descuento con fecha de creación correcta
    const [result] = await pool.query(
      "INSERT INTO descuentos (producto_id, porcentaje, fecha_inicio, fecha_fin, fecha_creacion) VALUES (?, ?, ?, ?, ?)",
      [producto_id, porcentaje, fecha_inicio, fecha_fin, fechaActual],
    )
...
export const updateDescuento = async (req, res) => {
  ...
  try {
    ...
    // Validar las fechas
    const fechaInicio = fecha_inicio ? new Date(fecha_inicio) : new Date(descuentos[0].fecha_inicio)
    const fechaFin = fecha_fin ? new Date(fecha_fin) : new Date(descuentos[0].fecha_fin)
    ...
    await pool.query("UPDATE descuentos SET porcentaje = ?, fecha_inicio = ?, fecha_fin = ?, activo = ? WHERE id = ?", [
